...
public class MapController : MonoBehaviour { //, ISave, IGameConnecter {
    ...
    private void Start() {
        ...
        SerializedDictionary<string, float> details = new() {
            { "First", 1 },
            { "Second", 2 }
        };
        
        S_Civilization civilization = new();
        civilization.Population = 1000;
        civilization.TakenFood = 100;
        civilization.Set("Paramiters1", details);
        civilization.Set("Paramiters2", details);

        S_Region region = new();
        region.Color = Color.blue;
        region.AddNeighvour("Ru");
        region.AddNeighvour("Eng");
        region.AddNeighvour("De");
        region.Set("Terrain", new SerializedDictionary<string, float>(){
            { "Terrain 1", 100},
            { "Terrain 2", 100}
        });
        region.Set("Flora", "Flora", 100);
        region.Set("Fauna", "Fauna", 100);
        region.Set("Unciv", civ);
