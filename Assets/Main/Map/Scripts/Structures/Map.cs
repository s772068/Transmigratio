...
[Serializable]
public class Map {
    [SerializeField] private SerializedDictionary<float, Civilization> civilizations = new();
    [SerializeField] private List<Region> _regions = new();

    public Action<float> OnUpgradeCivilization;

    public Region GetRegion(int index) => _regions[index];
    ...
    public Civilization GetCivilization(int id) => civilizations[id];
    public Civilization GetRandomCivilization() => civilizations.Values.ToArray()[Randomizer.Random(civilizations.Count)];
    ...
    public Color GetColor(int region) => _regions[region].Color;
    public string GetName(int region) => _regions[region].Name;


    // OnSetColor?.Invoke(region, GetColor(region), value);
    public void SetName(int region, string value) => _regions[region].Name = value;
    public void SetColor(int region, Color value) => _regions[region].Color = value;
    ...
    //public Paramiter[] GetEcologyParamiters(int region) => _regions[region].GetEcologyParamiters();
    //public float[] GetEcologyDetails(int region, int paramiter) => _regions[region].GetEcologyDetails(paramiter);


    //public Paramiter GetEcologyParamiter(int region, int index) => _regions[region].GetEcologyParamiter(index);
    //public float GetEcologyDetail(int region, int paramiter, int detail) => _regions[region].GetEcologyDetail(paramiter, detail);


    //public void SetEcologyParamiter(int region, int paramiter, Paramiter value) => _regions[region].SetEcologyParamiter(paramiter, value);
    ...
    public void SetEcologyParamiters(int region, Paramiter[] paramiters) => _regions[region].SetEcologyParamiters(paramiters);
    //public void SetEcologyDetails(int region, int paramiter, float[] details) => _regions[region].SetEcologyDetails(paramiter, details);


    //public void AddEcologyParamiter(int region, Paramiter value) => _regions[region].AddEcologyParamiter(value);
    //public void AddEcologyDetail(int region, int paramiter, float value) => _regions[region].AddEcologyDetail(paramiter, value);


    //public void RemoveEcologyParamiter(int region, Paramiter value) => _regions[region].RemoveEcologyParamiter(value);
    ...
    public float GetReserveFood(float civID) => civilizations[civID].ReserveFood;
    public float GetGovernmentObstacle(float civID) => civilizations[civID].GovernmentObstacle;

    //public void UpgradeCivilization(float civID) {
    //    if(civID < 1) {
    //        if (!civilizations.ContainsKey(1)) {
    //            Civilization civ = new();
    ...
    public void SetReserveFood(float civID, float value) => civilizations[civID].ReserveFood = value;
    public void SetGovernmentObstacle(float civID, float value) => civilizations[civID].GovernmentObstacle = value;
    ...
    //public void AddCivilizationParamiter(float civID, Paramiter value) {
    //    if (civilizations == null) civilizations = new();
    //    if (!civilizations.ContainsKey(civID)) civilizations.Add(civID, new());
