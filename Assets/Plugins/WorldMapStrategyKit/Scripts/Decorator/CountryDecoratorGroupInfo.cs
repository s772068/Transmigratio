			...
			for (int k = 0; k < decorators.Count; k++) {
				...
				if (countryIndex >= 0) {
					...
					if (active) {
						...
						// custom label
						if (decorator.labelOverride && ((country.customLabel == null && decorator.customLabel.Length > 0) || (country.customLabel != null && country.customLabel != decorator.customLabel))) {
						...
						} else if (!decorator.isColorized && mainRegion.customMaterial != null && mainRegion.customMaterial.color == decorator.fillColor) {
