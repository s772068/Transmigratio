#ifndef WMSK_TERRAIN_SPLATMAP_COMMON_CGINC_INCLUDED
#define WMSK_TERRAIN_SPLATMAP_COMMON_CGINC_INCLUDED
...
//	UNITY_FOG_COORDS(5)
	float2 wmskUV : TEXCOORD6;
};

sampler2D _Control;
float4 _Control_ST;
sampler2D _Splat0,_Splat1,_Splat2,_Splat3;
sampler2D _WMSK_Overlay;
float4 _WMSK_Data;
float4x4 _WMSK_Clip;
...
void SplatmapVert(inout appdata_full v, out Input data)
{
	...
	float3 mpos = float3(worldPos.x, 0, worldPos.z) + _WMSK_Data.xyz;
	float4 wmpos = mul(_WMSK_Clip, float4(mpos, 1.0));
	float4 scrPos = ComputeScreenPos(wmpos);
	data.wmskUV = scrPos.xy / scrPos.w;
	#if UNITY_UV_STARTS_AT_TOP
    	if (_ProjectionParams.x < 0) {
			data.wmskUV.y = 1.0 - data.wmskUV.y;
		}
	#endif
...
#else
void SplatmapMix(Input IN, out half4 splat_control, out half weight, out fixed4 mixedDiffuse, inout float3 mixedNormal)
#endif
{
	...
	float4 wmskOverlay = tex2D(_WMSK_Overlay, IN.wmskUV);
	float2 rangeUV = abs(IN.wmskUV.xy - 0.5.xx);
	clip(0.5.xx - rangeUV);

	mixedDiffuse = lerp(mixedDiffuse, wmskOverlay, _WMSK_Data.w * wmskOverlay.a);
	mixedDiffuse.a = 0; // clear smoothness
